...
function App() {
  ...
  useEffect(() => {
    ...
    connection.onmessage = (event) => {
      ...
      if (data.type === 'message') {
        ...
        if (data.message.from !== 'me' && data.message.from !== selectedIdRef.current) {
          setUnreadCounts((current) => ({ ...current, [data.message.from]: (current[data.message.from] || 0) + 1 }))
          if ('Notification' in window && Notification.permission === 'granted') new Notification('New Chaty message')
        }
      }
      if (data.type === 'message-edited') setMessages((current) => current.map((item) => item.id === data.messageId ? { ...item, text: data.text, edited: true } : item))
      if (data.type === 'message-deleted') setMessages((current) => current.map((item) => item.id === data.messageId ? { ...item, text: 'Message deleted', deleted: true } : item))
  ...
  function respondToRequest(requestId, status) {
    if (socket?.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: 'respond-request', requestId, status }))
  }
